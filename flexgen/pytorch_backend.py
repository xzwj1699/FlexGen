...
class LlamaTorchDevice(TorchDevice):
    ...
    def llama_mha(self, inputs, position_ids, attention_mask, w_ln, w_q, w_k, w_v,
        ...
        # k = k.permute(2, 0, 1)
        # v = v.permute(1, 0, 2)
        k = k.permute(2, 0, 1, 3).reshape(s, b*n_head, head_dim)
        v = v.permute(2, 0, 1, 3).reshape(s, b*n_head, head_dim)
        ...
        # print(f"Inferemce time cost, before attention calculation: {(time_2 - time_1) * 1000}ms, attention time: {(time_3 - time_2) * 1000}ms, after attention calculation: {(time_4 - time_3) * 1000} ms")
