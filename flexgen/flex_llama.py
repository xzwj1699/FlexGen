"""
Usage:
python3 -m flexgen.flex_llama --model meta-llama/Llama-2-7b-chat-hf --path path/to/your/model --gpu-batch-size 32 --percent 100 0 100 0 100 0
"""
import os
import torch
import argparse
...
from flexgen.llama_config import LlamaConfig, get_llama_config, download_llama_weights, check_format, convert_llama_weights
...
class LlamaLM(OptLM):
    ...
    def init_weight(self, j):
        ...
        has_model, has_np = check_format(self.path)
        if has_model and not has_np:
            convert_llama_weights(self.config.name, self.path)
        else:
...
def run_flexgen(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.path, token=args.hf_token, padding_side="left")
    # Original
    ...
    # Task and policy
    warmup_inputs = get_test_inputs(32000, num_prompts, tokenizer)
